...
class SectionedMultiSelect extends PureComponent {
  ...
  _displaySelectedItems = () => {
    const {
      ...
      /**
       * Laxaar - Add a way to allow custom rendering of selected Items
       */
    if(renderSelectedItems instanceof Function){
      return renderSelectedItems(selectedItems.map((singleSelectedItem) => {
          const item = this._findItem(singleSelectedItem);
          return item;
      }));
    }
    const { styles, colors } = this.state;
    const items= selectedItems.slice(0,selectedItemsDisplayCount||selectedItems.length).map((singleSelectedItem) => {
      const item = this._findItem(singleSelectedItem);
    ...
    if(items.length<selectedItems.length){
      return (
          <View style={{flexDirection:'row'}}>
              {items}
              <Text style={{fontSize:12,color:'#333',marginTop:12,marginLeft:10}}>
