  ...
  class ImageProgress extends Component {
    ...
    render() {
      ...
      //Added by laxaar
          ...
          if (renderIndicator) {
              indicatorElement = renderIndicator(0, true);
          } else {
              const IndicatorComponent =
                  typeof indicator === 'function' ? indicator : DefaultIndicator;
              indicatorElement = (
...
//Added by laxaar
          ...
          if (renderIndicator) {
              indicatorElement = renderIndicator(this.props.progress, false);
          } else {
              const IndicatorComponent =
                  typeof indicator === 'function' ? indicator : DefaultIndicator;
              indicatorElement = (
      ...
      const showLoadingStyle = (this.props.loading || (this.props.progress && this.props.progress<1));

      return (
        <View style={[style,showLoadingStyle?loadingStyle:null]} ref={this.handleRef}>
          <ImageComponent
            {...props}
            key={sourceKey}
